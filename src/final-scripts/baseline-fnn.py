...
# model.load_state_dict(torch.load('models/fnn_baseline_24_03_2024/best_model_fnn_baseline_epoch=1250.pth', map_location=device))
...
class ConservationCriterion(nn.Module):
    ...
    def forward(self, inputs, preds, targets):
        ...
        magnitude_diff = 0
        if mse_loss < constraint_loss:
            magnitude_diff = (torch.log10(constraint_loss) - torch.log10(mse_loss)).to(torch.long).to(torch.float64)
        
        new_mse_loss = mse_loss * 10**magnitude_diff

        total_loss = new_mse_loss + constraint_loss
...
class CustomCriterion(nn.Module):
    ...
    def forward(self, inputs, preds, targets):
        ...
        # clamp preds
        preds = torch.clamp(preds, min=-15, max=15)
...
run = Run(experiment="fnn-baseline")
...
run["hparams"] = {
    ...
    "loss": "SmoothL1"# + Conservation with Weight Decay, MSE magnitude weighted"
}
...
#lambda_val = 1e-10
#criterion.update_lambda(lambda_val)
...
for epoch in range(n_epochs):
    ...
    if epoch % look_back == 0 and epoch != 0:
        torch.save(best_model_ckpt, f'best_model_fnn_baseline_epoch={epoch}.pth')
        
    """if epoch % 2000 == 0:
        lambda_val = lambda_val * 2
        print(f"Updated lambda value to {lambda_val}")
        criterion.update_lambda(lambda_val)
...
# save the best model
torch.save(best_model, 'best_model_fnn_baseline.pth')

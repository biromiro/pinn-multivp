...
def load_model(model_path):
    ...
    if model_path == 'baseline':
        model.load_state_dict(torch.load(
            'best_model_baseline.pth', map_location=device))
    elif model_path == 'mass':
        model.load_state_dict(torch.load(
            'best_model_mass_conservation.pth', map_location=device))
    elif model_path == 'momentum':
        model.load_state_dict(torch.load(
            'best_model_momentum_conservation.pth', map_location=device))
    elif model_path == 'mass_momentum':
        model.load_state_dict(torch.load(
...
parser.add_argument(
    '-m', '--model',  type=str, help='Model Type: baseline, mass, momentum, mass_momentum', required=True)
